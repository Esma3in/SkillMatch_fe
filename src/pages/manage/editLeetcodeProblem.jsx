...
const EditLeetcodeProblem = () => {
  ...
  useEffect(() => {
    // Fetch problem data, skills and challenges on component mount
    const fetchData = async () => {
      try {
        setLoading(true);
        const [problemResponse, skillsResponse, challengesResponse] = await Promise.all([
          api.get(`/leetcode/problems/${id}`),
          api.get("/skills/all"),
          api.get("/challenges")
        ]);
        ...
        setFormData({
          ...
          skill_id: problem.skill_id.toString(),
          challenge_id: problem.challenge_id ? problem.challenge_id.toString() : "",
          examples: Array.isArray(problem.examples) ? problem.examples : [],
          test_cases: Array.isArray(problem.test_cases) ? problem.test_cases : [],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formattedTestCases = formData.test_cases.map(testCase => {
        ...
        if (typeof testCase.expected_output === 'object') {
          // If it's already an object, use it directly
          cleanedTestCase.expected_output = testCase.expected_output;
        } else if (testCase.expected_output && testCase.expected_output.trim()) {
          try {
        ...
        if (testCase.target && testCase.target.toString().trim()) {
      ...
      const response = await api.put(`/leetcode/problems/${id}`, problem);
      
      console.log("Problem updated:", response.data);
      setSuccess(true);
      
      // Navigate back to the problems list after a short delay
      setTimeout(() => {
        navigate("/training/problems");
      }, 2000);
      ...
      <div className="min-h-screen bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Edit LeetCode Problem</h1>
          </div>
          
          {/* Success message */}
          {success && (
            <div className="mb-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-700">
              <p className="font-medium">Success! The problem was updated successfully.</p>
            </div>
          )}
                ...
                <div key={index} className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <textarea
                        value={typeof testCase.expected_output === 'object' ? JSON.stringify(testCase.expected_output) : testCase.expected_output}
...
export default EditLeetcodeProblem; 